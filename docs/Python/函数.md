...
这个表达式是一个函数`lambda_f`，这个函数会返回一个把自己的参数作为其中一个参数的函数，它相当于：
...
当使用`lambda_f(factorial)(n)`时，它会先返回`lambda_x`，然后运行`lambda_x(n)`，即`factorial(factorial, n)`从而使得参数f始终为`factorial`函数本身，实现递归调用。

## 变量

Python 查找变量时，会按照这个固定的顺序（LEGB）：

- L (Local) 局部作用域：当前函数 __init__ 内部。

- E (Enclosing) 嵌套作用域：如果函数套在另一个函数里。

- G (Global) 全局作用域：模块（文件）顶层定义的变量。

- B (Built-in) 内置作用域：Python 自带的函数（比如 len, str）。

- 注意：这个列表中没有“类作用域” (Class Scope)。